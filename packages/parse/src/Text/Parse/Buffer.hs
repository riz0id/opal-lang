...
import Data.Unicode (readUtf8OffPtr, sizeofCharUtf8, writeUtf8OffPtr)
...
eqOffForeignPtr fp1 fp2 len = do
  ordering <- compareOffForeignPtr fp1 fp2 len
  pure (EQ == ordering)
...
stringToBuffer str = do
  ...
  withBufferContents buffer (`run` str)
  pure buffer
  where
    size :: Int
    size = foldr ((+) . sizeofCharUtf8) 0 str

    run :: Ptr Word8 -> String -> IO ()
    run _   []       = pure ()
    run ptr (x : xs) = do
